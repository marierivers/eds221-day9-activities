...
Reprex
```{r}
df_1 <- tribble(~Name, ~Height, ~Hair_color,
              "Teddy", 2.2, "brown",
              "Khora", 3.1, "white",
              "Waffle", 1.0, "tan")
...
```{r}
# Example 2:
creek_graph <- mack_creek_vertebrates %>% 
  clean_names() %>% 
  filter(species == "ONCL")

my_plot <- ggplot(data = creek_graph, aes(x = length1, y = weight)) +
  geom_point(shape = 12, fill = "purple") +
  theme_minimal() +
...
Reprex 2
```{r}
df <- tribble(~fish_id, ~length, ~weight,
              "fish_1", 2.2, 5,
              "fish_2", 3.1, 6,
              "fish_2", 1.0, 7)

my_plot <- ggplot(data = df, aes(x = length, y = weight)) +
  geom_point(shape = 12, fill = "purple") +
  theme_minimal() +
  labs(x = "Cutthroat trout length (cm)",
       y = "Weight (g)")
